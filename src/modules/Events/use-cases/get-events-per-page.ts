  ...
  async execute(pageNumber) {
    ...
    try {
      ...
      const formattedItems = await Promise.all(
        ...
        tribeEventsContainer.map((el) =>
          el.evaluate((el) => {
            ...
            const eventDateContent = a.querySelector(".event-date");
            ...
            const dateEnd =
              schedule.querySelector(".tribe-event-date-end") &&
              schedule.querySelector(".tribe-event-date-end").textContent;
            const time =
              schedule.querySelector(".tribe-event-time") &&
              schedule.querySelector(".tribe-event-time").textContent;
            ...
            const streetAddress =
              address.querySelector(".tribe-street-address") &&
              address.querySelector(".tribe-street-address").textContent;
            const locality =
              address.querySelector(".tribe-locality") &&
            ...
            const mapUrl =
              venueDetails.querySelector(".tribe-events-gmap") &&
      ...
      if (error instanceof this.scrapper.errors.TimeoutError) {
        throw new AppError({
          message:
            "[TIMEOUT] - Falha ao buscar eventos por página pois o tempo limite de requisição foi alcançado " +
            error,
          statusCode: 504,
        });
      }
      throw new AppError({
        message: `Falha ao realizar busca de eventos por página . Error -> ${error.message}`,
      });
    }
  }
}
