...
class ShowHighlightsNewsUseCase {
  ...
  async execute() {
    ...
    try {
      ...
      page.once("load", () =>
        console.log("Page inicial da UFC carregada com sucesso!")
      );
      ...
      const latestNews = await content.$eval("div:nth-child(1)", (el) => {
        ...
        items.forEach((item) => {
          //subtitulo
          const subtitleItem =
            item.querySelector(".subtitulo").firstElementChild;
          const title = {
      ...
      const contestAndSelections = await content.$eval(
        "#conteudo > div.ten.columns > div:nth-child(2)",
        (el) => {
          const result = [];
      ...
      throw new AppError({
        message: `Falha ao realizar  busca de notícias em destaques . Error -> ${error.message}`,
      });
    }
  }
}
