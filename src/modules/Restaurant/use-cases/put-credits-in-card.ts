...
export class PutCreditsInCardUseCase {
  ...
  async execute(
    ...
    try {
      ...
      await page.evaluate(
        (card_number, registry_number) => {
          ...
          let registrationInput =
            registration.lastElementChild.firstElementChild;
      ...
      const elementHandle = await page.$('iframe[id="iFrameResizer0"]');
      ...
      let payment = await frame.evaluate(() => {
        let paymentDetails = document.querySelector(
        ...
        let qrCodeCopy = details.querySelector(
          ".qr-code-copy-box .qr-code-box"
        );
